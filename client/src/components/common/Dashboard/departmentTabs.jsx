...
import { Check, ChevronsUpDown, Settings, Pencil, Trash2 } from "lucide-react"
...
export const HRDepartmentTabs = () => {
    ...
    return (
        <div className="flex flex-col gap-4 rounded-2xl shadow-sm ring-1 ring-gray-200/60 bg-white/80 backdrop-blur min-[250px]:p-1 sm:p-4 h-[100%] overflow-auto">
        ...
        <>
            ...
            <Tabs defaultValue="account" className="w-full h-[100%]">
                <div className="tabs-with-button flex justify-between items-center min-[250px]:flex-col-reverse sm:flex-row">
                    <TabsList className="min-[250px]:max-w-[250px] md:max-w-[300px] bg-gradient-to-r from-blue-50 to-purple-50 ring-1 ring-gray-200 text-blue-700 my-3 min-[250px]:flex min-[250px]:flex-col min-[250px]:py-14 min-[350px]:flex min-[350px]:flex-row min-[350px]:py-6">
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="min-[250px]:hidden sm:table-cell">Email</TableHead>
                                    <TableHead className="min-[250px]:hidden sm:table-cell">Contact Number</TableHead>
                                    <TableHead className="text-right">Remove Employee</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {CurrentDepartmentData?.employees?.map((item) => (
                                    <TableRow key={item._id}>
                                        ...
                                        <TableCell className="min-[250px]:hidden sm:table-cell">{item.email}</TableCell>
                                        <TableCell className="min-[250px]:hidden sm:table-cell">{item.contactnumber}</TableCell>
...
export const AllDepartments = ({ DepartmentData, SetCurrentDepartment }) => {
    return (
        <>
            {DepartmentData.data ? DepartmentData.data.map((department) => <div key={department.name} className="department-data rounded-2xl shadow-sm ring-1 ring-gray-200/60 bg-white/80 backdrop-blur p-4 flex flex-col gap-4 hover:shadow-md transition-shadow">
                <div className="department-heading-description flex justify-between items-center min-[250px]:items-center sm:items-start">
                    ...
                    <Button className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700" onClick={() => SetCurrentDepartment(department.name)}>View</Button>
                </div>
                <p className="font-bold min-[250px]:text-xs  sm:text-sm lg:text-lg min-[250px]:text-center sm:text-start">
