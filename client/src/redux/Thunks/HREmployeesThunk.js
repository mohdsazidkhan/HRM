...
export const HandleUpdateHREmployee = createAsyncThunk("HandleUpdateHREmployee", async (HREmployeeData, { rejectWithValue }) => {
    try {
        const { data } = HREmployeeData
        const response = await apiService.patch(`${HREmployeesPageEndPoints.UPDATE}`, data, { withCredentials: true })
        return { ...response.data, type: "EmployeeUpdate" }
    } catch (error) {
        return rejectWithValue(error.response.data);
    }
})